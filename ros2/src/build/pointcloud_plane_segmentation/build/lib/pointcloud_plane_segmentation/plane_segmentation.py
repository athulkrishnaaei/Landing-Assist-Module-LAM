...
class PlaneDetectionNode(Node):
    ...
    def pointcloud_callback(self, msg):
        ...
        if self.segmentation_method == "prosac":
            inlier_cloud = prosac_plane_segmentation(self,o3d_cloud)
        if self.segmentation_method == "ransac":
            inlier_cloud = ransac_plane_segmentation(self,o3d_cloud)
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description='Node for performing plane segmentation')
    parser.add_argument('segmentation_method', type=str,help='Plane segmentation method to perform')
    parsed_args = parser.parse_args()
    node = PlaneDetectionNode(parse_args.segmentation_method)
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
